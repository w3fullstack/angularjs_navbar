var myApp =   angular.module('myApp',['ngRoute', 'ngAnimate', 'ultimateDataTableServices', 'cc.autorefresh']);
console.log(myApp);

myApp.controller('navController', function ($scope, ShareScope){
    ShareScope.setScope($scope);
    $scope.isShow = true;
});
...
myApp.controller('jsonParseCtrl', function ($scope){
  // function to submit the form
  let data = {
    "finished": false,
    "dataType": "null",
    ...
    "rows": [{
      ...
      "currentValues": {
        ...
        "REVIEW_3": {
          ...
          "objectValue": "https://www.google.com "
        },
        "REVIEW_4": {
          "set": true,
          "unit": {
  ...
  let keys = [];
  let currentValues = [];
  $.each(data.rows[0].currentValues, function(key, value){
    keys.push(key);
    currentValues.push(value);
  });
  $scope.keys = keys;
  $scope.currentValues = currentValues;
});

myApp.controller('loginfCtrl',function($scope, $location, $http, ShareScope){
  alert('login');
    ShareScope.getScope().isShow = false;
	$scope.submit = function(){
		var uname = $scope.username;
		var pass = $scope.password;
		console.log("Logging in '" + uname + "' with password '" + pass + "'")

		if($scope.username == 'gale' && $scope.password == 'ABCDabcd1234gale') {
            ShareScope.getScope().isShow = true;
			$location.path('/home');
        }
    }
});
        
myApp.controller('tableController', ['$scope','datatable', '$timeout', function($scope,datatable, $timeout) {
    "use strict";
alert('table');
    var counter = 0,
    vm = this;
    ...
    //Simple example of configuration
        ...
        function refreshList(){
            datatableData[0].test = Math.random()*10;
            $scope.datatable = datatable(datatableConfig);
            //Set the data to the datatable
            $scope.datatable.setData(datatableData);
        }
        ...
        function initialise(){
            ...
            vm.list = datatableData;
        }
    
        initialise();
}]);

myApp.run(['$route', 'ShareScope', function($route, ShareScope)  {
    ShareScope.getScope().isShow = true;
}]);
...
myApp.config(['$routeProvider', '$locationProvider', function ($routeProvider, $locationProvider){
        $routeProvider
            ...
            .when('/bootstraptable',{
              ...
              templateUrl: 'pages/BootStrapTable.html'
            })
            .otherwise({
    ...
    myApp.factory('ShareScope', function () {
